...
def post_hook(predictions):
    ...
    for prediction in list(predictions):
        ...
        min_score = 1.
        for idx, (box, label_idx, score) in enumerate(zip(prediction.boxes.xywh, prediction.boxes.cls.cpu().tolist(), prediction.boxes.conf.tolist())):
            if score < min_score: min_score = score

            x_center, y_center, box_width, box_height = box.cpu().tolist()
            x = (x_center - box_width / 2) if width else 0
            ...
            result.append({
                ...
                'value': {
                    ...
                    'width': box_width,
                    'height': box_height,
                    'rectanglelabels': [prediction.names[label_idx]],
                    'score': score
                },
